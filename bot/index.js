...
// Дневной лимит расхода: реальная остановка работы, а не индикатор в /status.
// Возвращает true, если работать можно. Сама сообщает Олесе, если нет.
async function spendGuard(ctx) {
  const st = checkSpendLimit();
  const spent = getTodaySpend();
  const limit = state.dailySpendLimit || 50;
  if (st === "blocked") {
    await ctx.reply(
      `\u{1F534} <b>Дневной лимит исчерпан</b>\n\n` +
      `Потрачено сегодня: $${spent.toFixed(2)} из $${limit}.\n\n` +
      `Работа остановлена, чтобы расход не рос дальше. ` +
      `Поднять лимит: /settings \u2192 \u{1F4B0} Лимит. Завтра счётчик обнулится сам.`,
      { parse_mode: "HTML" }
    ).catch(() => {});
    return false;
  }
  const today = new Date().toISOString().slice(0, 10);
  if (st === "warning" && state.spendWarnedOn !== today) {
    state.spendWarnedOn = today;
    saveState(state);
    await ctx.reply(
      `\u{1F7E1} Израсходовано $${spent.toFixed(2)} из $${limit} за сегодня — больше 80% лимита. ` +
      `При достижении $${limit} работа остановится. Поднять: /settings \u2192 \u{1F4B0} Лимит.`
    ).catch(() => {});
  }
  return true;
}
...
class StatusMessage {
  ...
  // Показывает, что Агент делает прямо сейчас: последние 4 шага и таймер.
  async updateAction(phrase) {
    if (this.stopped || !phrase) return;
    this.actionLog.push(phrase);
    if (this.actionLog.length > 4) this.actionLog.shift();
    this.actionText = this.actionLog.join("\n");
...
function callClaude(prompt, sessionId, { onText, onTool } = {}) {
  const p = _queue.then(() => _callClaudeInner(prompt, sessionId, { onText, onTool }));
  _queue = p.catch(() => {});
  return p;
}
...
// Claude в режиме stream-json присылает блоки tool_use — каждое действие Агента.
// Раньше бот их выбрасывал, поэтому в Telegram было видно только "Думаю…".
// Здесь превращаем действие в короткую понятную строку.
function describeToolUse(name, input) {
  ...
  switch (name) {
    case "Read":      return `\u{1F4D6} Читаю ${base(i.file_path)}`;
    case "Write":     return `\u{1F4DD} Пишу ${base(i.file_path)}`;
    case "Edit":      return `\u270F\uFE0F Правлю ${base(i.file_path)}`;
    case "NotebookEdit": return `\u270F\uFE0F Правлю ${base(i.notebook_path)}`;
    case "Bash":      return `\u2699\uFE0F Выполняю: ${cut(i.description || i.command, 50)}`;
    case "Glob":      return `\u{1F50D} Ищу файлы: ${cut(i.pattern, 40)}`;
    case "Grep":      return `\u{1F50D} Ищу по тексту: ${cut(i.pattern, 40)}`;
    case "WebSearch": return `\u{1F310} Ищу в интернете: ${cut(i.query, 45)}`;
    case "WebFetch":  return `\u{1F310} Читаю страницу: ${cut(i.url, 45)}`;
    case "Task":      return `\u{1F916} Подключаю помощника: ${cut(i.description, 40)}`;
    case "TodoWrite": return `\u{1F4CB} Обновляю план работы`;
    case "Skill":     return `\u{1F9E9} Беру навык: ${cut(i.skill, 35)}`;
    default:          return `\u{1F527} ${cut(name, 35)}`;
  }
}


// ─── АВТОВЫБОР МОДЕЛИ ───────────────────────────────────────────────────────
// Отдельная настройка state.autoModel (не трогаем state.mode — он у автора
// задуман про автономность действий). Включена — модель подбирается под задачу:
// тяжёлое (архитектура, исследование, разбор, код) — Opus, короткая болтовня —
// Haiku, всё остальное — Sonnet. Выключена — работает модель, выбранная руками.

const HEAVY_PATTERNS = /архитектур|спроектир|проектир|стратег|исследов|проанализ|анализ|разбор|разбер|сравн|оцен|обоснуй|продумай|план разработ|дорожн|roadmap|\bmvp\b|\bмцп\b|техническ\w* задани|\bтз\b|юридич|правов|лиценз|налог|риск|безопасн|уязвим|монетизац|воронк|конкурент|позициониров|рефактор|отладь|найди ошибк|напиши код|реализуй|интеграц|миграц/i;
const LIGHT_PATTERNS = /^(привет|здравствуй\w*|добрый (день|вечер|утро)|спасибо|благодарю|пока|ок|окей|хорошо|понятно|ага|да|нет|ясно|принято|супер|отлично)[\s!.,)]*$/i;

function pickModelForPrompt(text) {
  const s = String(text || "").trim();
  if (LIGHT_PATTERNS.test(s)) return "haiku";
  if (HEAVY_PATTERNS.test(s)) return "opus";
  if (s.length > 600) return "opus";
  if (s.length < 40 && !/\?/.test(s)) return "haiku";
  return "sonnet";
}

function resolveModelForPrompt(text) {
  return state.autoModel ? pickModelForPrompt(text) : getCurrentModel();
}

const MODEL_LABEL = { opus: "Opus", sonnet: "Sonnet", haiku: "Haiku" };

function _callClaudeInner(prompt, sessionId, { onText, onTool } = {}) {
  return new Promise((resolve, reject) => {

    const useStream = typeof onText === "function" || typeof onTool === "function";
    const chosenModel = resolveModelForPrompt(prompt);
    if (typeof onTool === "function") {
      const how = state.autoModel ? " (подобрана под задачу)" : "";
      try { onTool(`\u{1F9E0} Модель: ${MODEL_LABEL[chosenModel] || chosenModel}${how}`); } catch {}
    }
    const args = [
      "-p", prompt,
          ...
          try {
            ...
            if (obj.type === "assistant" && obj.message?.content) {
              for (const block of obj.message.content) {
                ...
                } else if (block.type === "tool_use" && typeof onTool === "function") {
                  try { onTool(describeToolUse(block.name, block.input)); } catch {}
                }
              }
...
function settingsKeyboard() {
  ...
  return new InlineKeyboard()
    ...
    .text(`${state.autoModel ? "🧠" : "✋"} Выбор модели: ${state.autoModel ? "автоматически" : "вручную"}`, "settings_automodel").row()
...
// Settings callbacks
bot.callbackQuery("settings_automodel", async (ctx) => {
  await ctx.answerCallbackQuery();
  state.autoModel = !state.autoModel;
  saveState(state);
  await ctx.editMessageText(
    state.autoModel
      ? "🧠 Модель подбирается под задачу.\n\nOpus — архитектура, исследования, разборы, код.\nHaiku — короткие реплики.\nSonnet — всё остальное.\n\nКакая поехала — видно в сообщении о ходе работы."
      : "✋ Модель выбирается вручную: " + (state.model || "sonnet") + ".\n\nСменить: /model",
...
bot.command("status", async (ctx) => {
  ...
  const status =
    ...
    `<b>Модель:</b> ${state.autoModel ? "подбирается под задачу" : (state.model || "sonnet")}\n` +
...
bot.callbackQuery("confirm_continue", async (ctx) => {
  await ctx.answerCallbackQuery();
  if (!(await spendGuard(ctx))) return;
  const userId = String(ctx.from.id);
  const sessionId = sessions.get(userId) || null;
...
async function handleTextMessage(ctx, text) {
  ...
  try {
    ...
    let result = await callClaude(enrichedPrompt, sessionId, {
      onTool: (phrase) => { status.updateAction(phrase); },
    });
...
bot.on("message:voice", async (ctx) => {
  ...
  try {
    ...
    const result = await callClaude(transcript, sessionId, {
      onTool: (phrase) => { if (_activeStatus) _activeStatus.updateAction(phrase); },
    });
